...
export async function GET(
  ...
  const transformer = await transformersStore.getById(Number(id));
  if (!transformer) {
    return NextResponse.json(
      { success: false, message: "Transformador no encontrado" },
...
export async function PATCH(
  ...
  try {
    ...
    const updated = await transformersStore.updateStatus(
      Number(id),
      body.status,
      body.availableCapacityKva
    );
    if (!updated) {
      return NextResponse.json(
        { success: false, message: "Transformador no encontrado" },
